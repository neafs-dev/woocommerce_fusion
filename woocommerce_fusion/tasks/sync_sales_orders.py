...
def run_sales_order_sync(
    ...
    if not any([sales_order_name, sales_order, woocommerce_order_name, woocommerce_order]):
        raise ValueError("At least one of sales_order_name, sales_order, woocommerce_order_name, woocommerce_order is required")
    ...
    if woocommerce_order or woocommerce_order_name:
        if not woocommerce_order:
            woocommerce_order = frappe.get_doc({"doctype": "WooCommerce Order", "name": woocommerce_order_name})
...
class SynchroniseSalesOrder(SynchroniseWooCommerce):
    ...
    def update_sales_order(self, woocommerce_order: WooCommerceOrder, sales_order: SalesOrder):
        ...
        if sales_order.docstatus != 2:
            ...
            payment_method = woocommerce_order.payment_method_title if len(woocommerce_order.payment_method_title) < 140 else woocommerce_order.payment_method
    ...
    def update_woocommerce_order(wc_order: WooCommerceOrder, sales_order: SalesOrder) -> None:
        ...
        # Update the woocommerce_status field if necessary
        sales_order_wc_status = WC_ORDER_STATUS_MAPPING[sales_order.woocommerce_status] if sales_order.woocommerce_status else None
        ...
        if wc_server.sync_so_items_to_wc:
            ...
            if sales_order_items_changed:
                ...
                replacement_line_items = [{"id": line_item["id"], "product_id": None} for line_item in json.loads(wc_order.line_items)]
                # Add the correct lines
                replacement_line_items.extend([{"product_id": so_item.woocommerce_id, "quantity": so_item.qty, "price": so_item.rate} for so_item in sales_order.items])
    ...
    def create_sales_order(self, wc_order: WooCommerceOrder) -> None:
        ...
        payment_method = wc_order.payment_method_title if len(wc_order.payment_method_title) < 140 else wc_order.payment_method
        ...
        if (wc_server.enable_shipping_methods_sync) and (shipping_lines := json.loads(wc_order.shipping_lines)) and len(wc_server.shipping_rule_map) > 0:
            if len(wc_order.shipping_lines) > 0:
                shipping_rule_mapping = next(
                    (rule for rule in wc_server.shipping_rule_map if rule.wc_shipping_method_id == shipping_lines[0]["method_title"]),
                    None,
                )
    ...
    def create_or_link_customer_and_address(self, wc_order: WooCommerceOrder) -> str:
        ...
        existing_customer = frappe.get_value("Customer", {"woocommerce_identifier": customer_identifier}, "name")
    ...
    def create_missing_items(self, wc_order, items_list, woocommerce_site):
        ...
        for item_data in items_list:
            ...
            if item_woo_com_id != "0":
                woocommerce_product_name = generate_woocommerce_record_name_from_domain_and_id(woocommerce_site, item_woo_com_id)
        ...
        for item in json.loads(wc_order.line_items):
            ...
            if woocomm_item_id == 0:
                ...
                item_codes = (
                    ...
                    .where((iws.woocommerce_id == cstr(woocomm_item_id)) & (iws.woocommerce_server == new_sales_order.woocommerce_server) & (itm.disabled == 0))
                    .select(iws.parent)
                    .limit(1)
                ).run(as_dict=True)
            ...
            if not wc_server.use_actual_tax_type:
                tax_template = frappe.get_cached_doc("Sales Taxes and Charges Template", wc_server.sales_taxes_and_charges_template)
    ...
    def create_or_update_address(self, wc_order: WooCommerceOrder):
        ...
        """
        addresses = get_addresses_linking_to("Customer", self.customer.name, fields=["name", "is_primary_address", "is_shipping_address"])
        ...
        address_keys_same = [True if raw_billing_data[key] == raw_shipping_data[key] else False for key in address_keys_to_compare]
    ...
    def create_address(self, raw_data: Dict, customer, address_type, is_primary_address=0, is_shipping_address=0):
        title_convention = frappe.db.get_value("WooCommerce Server", self.woocommerce_order.woocommerce_server, "address_title_convention")
        ...
        address.address_title = customer.customer_name if title_convention == "Customer Name only" else f"{customer.name}-{address.address_type}"
...
def get_list_of_wc_orders(
    ...
    while new_results:
        ...
        new_results = woocommerce_order.get_list(args={"filters": filters, "page_lenth": page_length, "start": start, "as_doc": True})
...
def get_tax_inc_price_for_woocommerce_line_item(line_item: Dict):
    ...
    """
    return (float(line_item.get("subtotal")) + float(line_item.get("subtotal_tax"))) / float(line_item.get("quantity"))
